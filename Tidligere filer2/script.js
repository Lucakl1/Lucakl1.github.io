...
function toggle_popup(){
    if(switchs.style.transform == "rotate(-45deg) translate(-340px, 230px)"){
        close_popup();
    }
    else{
        open_popup();
...
//Cubes
const cube_box2 = document.querySelectorAll(".cubes2");
const cube_box1 = document.querySelectorAll(".cubes1");
const width = window.innerWidth;
const white1 = width / 3;
...
for (let i = 0; i < cube_box2.length; i++) {
    for (let ii = 1; ii < Math.floor((width-30)/(cube_width+space_between)); ii++) {
        cube_box2[i].innerHTML += '<div class="cube"></div>';
    }
}

for (let i = 0; i < cube_box1.length; i++) {
    for (let ii = 1; ii < Math.floor((width-30)/(cube_width+space_between)); ii++) {
        if(white1 < (cube_width+space_between)*ii){
            cube_box1[i].innerHTML += '<div class="white"></div>';
        }
        else{
            cube_box1[i].innerHTML += '<div class="cube"></div>';
        }
    }
}
